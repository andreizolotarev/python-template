...
- Output ONLY the commit message text: a concise subject line (imperative mood) followed by an optional body explaining the why.
- Use Conventional Commits prefixes (e.g. `feat:`, `fix:`, `docs:`, `refactor:`, `chore:`) where relevant, since the project uses release-please.
- No code fences, no commentary, no explanations — so the user can copy the message directly into VS Code's source control box or a `git commit -m` command.
