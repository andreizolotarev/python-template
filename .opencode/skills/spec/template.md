...
- Typing: prefer `dataclasses`/`typing`; avoid `Any` unless justified.
```
...
- [ ] The module imports without errors (`python -c "import <package>"`).
- [ ] Calling `add_score(...)` increases the total by exactly 10.
- [ ] State persists across process restarts (file/SQLite).
```
...
- **Yes:** a JSON file for persistence. Fits in <5MB and we don't need queries.
- **No:** SQLite. Overengineering for this case (no relational needs).
...
| Risk                                  | Mitigation                                                                  |
| ------------------------------------- | --------------------------------------------------------------------------- |
| Storage file missing or unreadable   | Fallback to in-memory object. The app still runs, it just doesn't persist.  |
| Future incompatible schema            | Key includes `:v1`. Migration documented in `persistence.py`.               |
```
...
- **Concrete names.** If you say "the levels module", say `src/levels.py`. If you say "a key", give the exact string.
